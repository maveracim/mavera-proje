...
exports.run = async(client, msg, args, yes, no, embed, uye) => {
    ...
    if(!nick) return msg.reply({content:`Bir isim ve yaş belirt. \`${base.prefixes.moderation}isim @Mavera/ID isim yaş\``})
    if(!age || isNaN(age)) return msg.reply({content:`Bir yaş belirt. \`${base.prefixes.moderation}isim @Mavera/ID isim yaş\``})
...
${isimler.isimler.reverse().join("\n")}`)]})
        ...
        await Names.findOneAndUpdate({ uye:uye.user.id }, { $push:{ isimler:`\`${db.tag} ${nick} | ${age}\` (İsim Değişikliği)` } }, { upsert:true })
    })
}
exports.mavera = { name: "isim", ek:["nick", "i"] }