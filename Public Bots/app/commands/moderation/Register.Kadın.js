...
exports.run = async(client, msg, args, yes, no, embed, uye) => {
    ...
    if(!nick) return msg.reply({content:`Bir isim ve yaş belirt. \`${base.prefixes.moderation}kadın @Mavera/ID isim yaş\``})
    if(!age || isNaN(age)) return msg.reply({content:`Bir yaş belirt. \`${base.prefixes.moderation}kadın @Mavera/ID isim yaş\``})
    await uye.setNickname(`${db.tag} ${nick} | ${age}`).then(async() => {
        await uye.roles.set(db.kadın)
        await Stats.findOneAndUpdate({ uye:msg.member.id },{$inc:{topKayıt:1,kadın:1,coin:5}},{upsert:true})
...
${isimler.isimler.reverse().join("\n")}`)]})
        ...
        await Names.findOneAndUpdate({ uye:uye.user.id }, { $push:{ isimler:`\`${db.tag} ${nick} | ${age}\` (${db.kadın.map(x => `<@&${x}>`).join(", ")})` }, $inc:{ toplam:1 } }, { upsert:true })
    })
}
exports.mavera = { name: "kadın", ek:["k", "kız", "woman", "women"] }